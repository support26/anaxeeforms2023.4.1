...
const testfunc =(partial) => {
  // console.log("partial-------------------",partial.xls)
  const auxiliaryKey = Object.getOwnPropertySymbols(partial).find(sym => String(sym) === 'Symbol(auxiliary)');
  const auxiliaryValue = partial[auxiliaryKey];
  if (auxiliaryValue.formData) {
  auxiliaryValue.formData.xmlFormId = partial.name.replace(/\n/, '');
  // console.log("auxiliaryValue-------------------",auxiliaryValue.formData)
  const result =  axios.post('https://anaxeeform.azurewebsites.net/api/formHandler?code=twUpf2vU8b3IRuopeEYUipqo94vzPLPV8UKoDnfQ-seBAzFui8l1Dg==', auxiliaryValue.formData);
...
const sendDataToAirtable = async (form) => {
  try {
    // console.log("form-----",form);
    const [fields] = await Promise.all([
        getFormFields(form.xml),
    ]);
    // const datasetName = parser.parse(form.xml);
    ...
    const result =  axios.post('https://anaxeeform.azurewebsites.net/api/formHandler?code=twUpf2vU8b3IRuopeEYUipqo94vzPLPV8UKoDnfQ-seBAzFui8l1Dg==', data);
    console.log("result-------------------",result)
    return form;
  } catch (error) {
